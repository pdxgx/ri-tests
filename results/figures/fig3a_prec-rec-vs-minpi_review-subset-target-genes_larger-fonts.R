...
# Sys.setenv(axis_text_x = element_text(size = 5))
...
lgg <- lapply(ldfpr, function(dfpr){
  ...
  gg.ptline.prec <- ggplot(dfpr, aes(x = min.lr, y = precision, color = Tool)) + 
    geom_point(alpha = alpha.val, size = pt.size) + 
    geom_line(alpha = alpha.val, size = line.size) + 
  ...
  # prep multiplot
  gg.ptline.prec <- gg.ptline.prec + 
    theme(axis.title.x = element_blank(), legend.position = "none")
  gg.ptline.rec <- gg.ptline.rec + 
    theme(axis.title.x = element_blank(), legend.position = "none")
  gg.ptline.f1score <- gg.ptline.f1score + 
    theme(axis.title.x = element_blank(), legend.position = "none")
...
{
  prec.hx1 <- lgg[["HX1"]]$prec + ylim(0, ymax.prec) + 
    xlim(xlim.min, xlim.max) + ggtitle("HX1") +
  ...
  rec.hx1 <- lgg[["HX1"]]$rec + ylim(0, ymax.rec) + 
    xlim(xlim.min, xlim.max) +
    theme(axis.title.x = element_blank(), 
          axis.text.x = element_blank(),
          axis.title.y = element_text(size = yaxis.title.size),
          axis.text.y = element_text(size = yaxis.text.size))
  f1.hx1 <- lgg[["HX1"]]$f1score + ylim(0, ymax.f1) + 
    xlim(xlim.min, xlim.max) +
    theme(axis.title.x = element_blank(),
          axis.title.y = element_text(size = yaxis.title.size),
          axis.text.y = element_text(angle=0,size=yaxis.text.size),
          axis.text.x = element_text(angle=0,size=xaxis.text.size))
  prec.ipsc <- lgg[["iPSC"]]$prec + ggtitle("iPSC") +
    ylim(0, ymax.prec) + xlim(xlim.min, xlim.max) +
  ...
  rec.ipsc <- lgg[["iPSC"]]$rec + 
    ylim(0, ymax.rec) + xlim(xlim.min, xlim.max) + 
    theme(axis.title.y = element_blank(), 
          axis.title.x = element_blank(),
          axis.text.y = element_blank(), 
          axis.text.x = element_blank()) #+
  #scale_x_continuous(breaks = seq(xlim.min, xlim.max, 0.1))
  f1.ipsc <- lgg[["iPSC"]]$f1score + 
    ylim(0, ymax.f1) + 
    xlim(xlim.min, xlim.max) +
    theme(axis.title.y = element_blank(), 
          axis.title.x = element_blank(), 
...
xlab.maintitle.str <- paste0("Minimum persistence", paste0(rep(" ", 10), collapse = ""), 
                   collapse = "")
...
grid.arrange(prec.hx1, rec.hx1, f1.hx1, prec.ipsc, rec.ipsc, f1.ipsc,
             ...
             bottom=grid::textGrob(xlab.maintitle.str, 
                                   gp=grid::gpar(fontsize=15)))
dev.off()
# png
png.fname <- paste0(plot.fname, ".png")
png(png.fname, 7.5, 4, units = "in", res = 500)
grid.arrange(prec.hx1, rec.hx1, f1.hx1, prec.ipsc, rec.ipsc, f1.ipsc,
             as_ggplot(lgg[[1]]$legend), layout_matrix = lm,
             bottom=grid::textGrob(xlab.maintitle.str, 
                                   gp=grid::gpar(fontsize=15)))
