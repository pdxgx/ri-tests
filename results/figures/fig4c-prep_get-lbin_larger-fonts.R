...

{
  ...
  median.hx1 <- ggplot(dfii, aes(x = bin.min, y = log.median, color = `Truth\ncategory`)) + 
    ...
    theme(axis.title.x = element_blank(),
          ...
          axis.title.y = element_text(size = title.text.size),
          plot.title = element_text(size = title.text.size),
          legend.position = "none", 
          axis.text.x = element_blank(),
          plot.margin = unit(c(1.8, 1.8, 1.8, 3), "mm")) + 
    ggtitle("HX1") +
  ...
  lm <- matrix(c(rep(c(1,2), 5), rep(c(3,4), 3), rep(5, 4)), nrow = 2)
